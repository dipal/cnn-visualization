...
class GuidedBackprop(Backpropagation):
	...
	def guided_backprop_adjacent(self, layer_cur, layer_prev, values_prev, gate_b):
		...
		try:
			...
			print (type(values_prev), type(gate_f), type(gradients), type(loss), type(layer_prev), type(layer_cur), type(gate_b))
			raise e
	...
	def feed_forward(self):
		...
		forward_layers = [layer.get_output_at(-1) for layer in self.model.layers[1:self.layer_idx+1]]
